...
async function generate_and_display() {
    try {
        ...
        favoriteBtn.onclick = () => toggleFavorite(recipeData.recipe_id);
...
async function toggleFavorite(recipeId) {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || 'Erro ao favoritar receita');
        }
        ...
        if (data.is_favorite) {
            ...
            favoriteBtn.querySelector('img').src = favori"GET /nova-receita/{{%20url_for('static',%20filename='assets/favorite-filled.svg')%20}} HTTP/1.1" 404 -teBtn.dataset.favoriteFilleddIcon;
        } else {
