...
def test_prompt_segura_ingredientes_exclusivos():
    ...
    assert generate_message(contexto_seguro_1)=='''Se, por qualquer motivo, você não puder fornecer uma resposta, 
                                                sempre, sem exceções, responda com "Parece que você inseriu alguma 
                                                informação não válida, tente novamente!". Sem introduções e explicações 
                                                sobre, me dê uma receita que sirva 3 pessoa(as) para Café da Manhã, ela 
                                                deve ser da culinaria Vegana, a receita NÃO pode conter Leite, ela deve 
                                                usar os seguintes ingredientes: Arroz, Ovo, use apenas esses ingredientes 
                                                e nenhum outro, se necessário, de preferência use apenas os seguintes 
                                                eletrodomésticos: Airfryer'''
