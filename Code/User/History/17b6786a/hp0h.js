...
async function toggleFavorite(recipeId) {
    try {
        ...
        const favoriteBtn = document.getElementById("favoriteBtn");
        const favoriteIcon = favoriteBtn.dataset.favoriteIcon;
        const favoriteFilledIcon = favoriteBtn.dataset.favoriteFilleddIcon;
        
        if (data.is_favorite) {
            favoriteBtn.classList.add("text-orange-500");
            favoriteBtn.querySelector('img').src = favoriteFilledIcon;
        } else {
            favoriteBtn.classList.remove("text-orange-500");
            favoriteBtn.querySelector('img').src = favoriteIcon;
        }
    } catch (error) {
        console.error('Erro:', error);
        alert(error.message);
