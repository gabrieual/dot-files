...
async function toggleFavorite(recipeId) {
    try {
        ...
        if (data.is_favorite) {
            ...
            favoriteBtn.querySelector('img').src = "{{ url_for('static', filename='app/src/assets/favorite-filled.svg') }}";
        } else {
            favoriteBtn.classList.remove("text-orange-500");
            favoriteBtn.querySelector('img').src = "{{ url_for('static', filename='assets/favorite.svg') }}";
        }
    } catch (error) {
        console.error('Erro:', error);
        alert(error.message);
