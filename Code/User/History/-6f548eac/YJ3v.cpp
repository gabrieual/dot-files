...
public:
    ...
    void del(int key){
        ...
        for (size_t i = 0; i < arr[hashed_key].size(); i += 2) {
            if (arr[hashed_key][i] == std::to_string(key)) {
                // Remover chave e valor usando uma abordagem diferente
                std::vector<std::string> novo_vetor;
                for (size_t j = 0; j < arr[hashed_key].size(); j++) {
                    if (j != i && j != i+1) {
                        novo_vetor.push_back(arr[hashed_key][j]);
                    }
                }
                arr[hashed_key] = novo_vetor;
                load -= 1;
                return;
            }
        }
        // Se a chave não foi encontrada, ignorar operação
    }
};
